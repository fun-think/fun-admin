...
import { globalSearch } from '@/api/resources.js'
...
<template>
  <page-container title="全局搜索">
    <a-card>
      <a-space>
        <a-input v-model:value="keyword" placeholder="请输入关键词" style="width: 320px" @press-enter="handleSearch" />
      ...
      <div v-else>
        <div v-for="group in results" :key="group.slug" style="margin-bottom: 16px">
          <h3>{{ group.title }}</h3>
          <a-list :data-source="group.items" :render-item="item => itemRender(item, group.slug)" />
        </div>
      </div>
    </a-card>
  </page-container>
