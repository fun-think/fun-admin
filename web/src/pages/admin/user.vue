...
import { createUser, deleteUser, getUsers, updateUser } from '@/api/user.js'
...
async function onSubmit() {
  ...
  try {
    ...
    if (formModel.id > 0 && !formModel.changePassword) {
      delete formData.password
    }

    let res = {}
    if (formModel.id > 0) {
      res = await updateUser(formModel.id, formData)
    }
    else {
        ...
        <a-row :gutter="16">
          ...
          <a-col :span="24">
            <a-form-item label="分配角色" name="roles">
              ...
              >
                <a-select-option v-for="item in roleMap" :key="item" :value="item">
                  {{ item }}
                </a-select-option>
              </a-select>
            </a-form-item>
