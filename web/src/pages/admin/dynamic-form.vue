...
import { Button, Checkbox, DatePicker, Input, InputNumber, Progress, Select, Textarea, Upload, message } from 'ant-design-vue'
import {
...
import { getStorageConfig, uploadFile } from '@/api/file.js'
...
// 加载关联数据
async function loadRelationshipData(field) {
  try {
...
function handleRemove(fieldName) {
  fileList.value[fieldName] = []
    ...
    case 'file':
      return h(
        ...
        [
          ...
          uploadProgress.value[field.name] > 0 && uploadProgress.value[field.name] < 100
            ? h(
                Progress,
                {
