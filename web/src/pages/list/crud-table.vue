...
import { deleteCrudItem, getCrudTable } from '@/api/list/crud-table'
...
// 简化的状态管理，因为 useTableQuery 可能不存在
const state = ref({
  loading: false,
  dataSource: [],
  queryParams: {
...
async function query() {
  ...
  try {
    const res = await getCrudTable(state.value.queryParams)
    state.value.dataSource = res.data || []
    state.value.pagination.total = res.total || 0
  }
  catch (error) {
...
async function handleDelete(record) {
  ...
  try {
    const res = await deleteCrudItem(record.id)
    if (res.code === 200) {
